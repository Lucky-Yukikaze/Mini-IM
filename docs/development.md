...
当前接入范围为服务端。Qt 尚未发送 SyncApplied，也未消费 DeliveryUpdated；
现有 Qt 的未读消息将保持 `sent`，直到已读操作提供接收证据。
新协议的其他客户端若产生 DeliveryUpdated，当前 Qt 会因不支持该事件而停止同步；
应在下一批完成 Qt 事件持久化、确认重试和页面投递展示后再启用混合客户端联调。
协议修改后重新生成 Python 协议并重建 Qt，生成命令仍见本指南前文；
本批验证与边界见 [服务端收件确认批次](refactoring-progress.md#服务端收件确认与旧库升级--2026-09-10)。
...
文件取消检查在初始化被临时拒绝时进行，未覆盖文件内容完整传输。
