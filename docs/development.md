...
文件初始化与完成仍使用各自恢复路径，尚未全部接入统一请求 ID 约束。
...
文件完成覆盖提交前和确认丢失，已读与撤回覆盖各自提交后确认丢失。
...
验证不覆盖机器断电、磁盘损坏、取消与收件确认的双方共同退出、
默认 15 分钟上传接管等待或完整 Qt WebEngine 页面；实际结果见 [共同退出批次](refactoring-progress.md#客户端与服务共同退出恢复--2026-09-10)。
