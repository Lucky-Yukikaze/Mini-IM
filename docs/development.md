...
初次结果见 [独立桌面包批次](refactoring-progress.md#windows-独立桌面包与迁移验收--2026-09-12)，
当前源码的包与迁移交互复验见 [当前版本桌面交付](refactoring-progress.md#当前版本桌面交付--2026-09-13)。
...
固定展示窗口的原生前置接口 `loadHistoryPage(conversationId, cursor, direction)` 已提供三个方向：
`latest` 使用空游标返回最近 50 条，`older` 返回边界之前的至多 50 条（空游标等同最近页），
`newer` 必须提供游标，返回边界之后的至多 50 条；各方向都按会话内序号、消息 ID 升序返回。
结果附 `beforeCursor`、`afterCursor`、`hasOlder`、`hasNewer`，分别表示当前页两端和相应方向是否仍有消息；
旧的 `cursor`、`hasMore` 表示本次查询方向的继续位置和剩余状态，原 `loadHistory` 保持更早方向语义。
返回空页时双端游标保留请求边界，旧 `cursor` 为空；读取不改变同步位置或未读总数。
游标可以在同一账户缓存重开后继续使用；过长（超过 2048 个 QString 字符）、格式错误、跨会话边界及错误方向明确拒绝。
本接口仅扩展原生导航能力，当前聊天页面仍使用累加历史，尚未启用固定大小窗口和向更新方向的按钮。
...
真实回调等待、最早消息可见及历史按钮结束状态；另通过实际 QWebChannel 双向遍历全部 125 条，核对空页与非法方向边界；不伪造历史读取结果。
...
[测量入口](../web/tests/measure-session.mjs) 将 [状态驱动](../web/tests/session-benchmark.ts) 与生产页面状态打包，
为每个规模启动新的 Node 进程；需要先安装 `web/` 依赖。默认单会话、100 条一批，分别累积 100、1,000、10,000 条消息。
每条包含 128 字节正文、一项已读人数和一项投递，逐项核对唯一消息、顺序、正文长度、人数、投递及最终未读总数。

~~~powershell
node ./web/tests/measure-session.mjs
node ./web/tests/measure-session.mjs 0 101
~~~

位置参数为消息数，范围 0 至 10,000；单个测量进程超时 300 秒。失败记录错误并非零退出。
每批分别记录关联状态更新和消息合并耗时；最后单独测量 `currentMessages` 生成完整展示对象的耗时。
保留堆字节 = 完成数据核对并主动回收不可达对象后的堆用量减去初始化空会话后的堆用量；
采样进程驻留字节 = 每批结束与最终检查时 `process.memoryUsage().rss` 的最大值，可能漏掉批次内部峰值。
测量使用 `--expose-gc`，只在起止边界主动回收；输入生成、校验及主动回收时间均不计入合并或状态更新耗时。
结果包含全部批次原始数据、Node/系统/CPU 信息、源码及打包驱动摘要，保存在 `tmp/session-measurement/<UTC 时间>/`。

这项检查只测 Node 中的实际 Pinia 状态，不包含 Qt、网络、文件任务或页面绘制；不能把它当作 Qt WebEngine 的内存或渲染结果。
每个规模默认只有一个进程，不推断长期尾延迟；真实桌面功能继续使用 [桌面联调](#真实桌面页面联调)。
消息合并按批次更新会话数组，查找映射按数组身份复用；数组结构变化时必须替换数组，避免复用失效映射。
该映射不延长被替换数组的存活期；当前仍保留全部在线消息及关联状态，内存容量尚未受控。
实际测量及剩余工作见 [页面消息批量合并](refactoring-progress.md#页面消息批量合并与状态规模--2026-09-12)。
