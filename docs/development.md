...
共享核心或头文件修改后，必须先执行上述完整构建命令再运行 CTest；已有测试程序可能仍链接旧实现。
...
首次读取核对全部已保存消息，随后只计算受影响消息或本次新增已读区间的变化，提交成功后更新当前账号的内存总数；
事务失败不更新总数，重新打开或切换账号会重新计算，数据库结构及恢复位置保持原语义。
...
& ./.venv/Scripts/python.exe ./tools/measure_cache.py --event-count 100 --repeats 1
~~~
...
`--event-count` 默认 0，设为 1 至 1000 时，在快照、完整历史与旧库索引验证结束后，另启进程向第一个会话逐条提交指定数量的新消息。
每条调用生产 `apply()`，再调用 `unreadTotal()`，分别记录写入与读取耗时，逐条核对未读总数和连续位置；最终核对数据库消息数及完整性。
`events-<原数量>.json` 保存全部样本及进程时间，样本不计入初始快照测量；输入为单写入者、顺序调用，已打开缓存的首次统计不计入逐条样本。
原输入只填充对象表，历史事件去重表为空且同步位置为 0；新增事件从位置 1 开始，不代表同规模历史事件去重表的成本。
这项事件测量覆盖本地提交和派生计数，不包含同步协调、网络或真实消息到达率；结果见 [增量未读统计](refactoring-progress.md#增量未读统计与事件成本--2026-09-12)。
