...
其中不含协议字节；页面等待本地保存回调，保存失败保留输入，保存后显示待确认或确定失败状态。
原已读请求确认后，页面可提交后来读到的消息；传输重试仍由 Qt 协调。
页面交互与恢复的具体覆盖范围见执行记录。
...
文件控制使用 [FileInit / FileFinish / FileCancel / FileUpdated](proto/file.proto)，文件内容使用独立 QUIC 流。
...
上传以服务端实际文件与已提交记录共同确定的位置续传；缺失/截短时回退，未提交尾部被截去，
每批数据完整写入并请求落盘后才提交进度，修正位置与单调版本的同步事件共同保存。
上传完成确认等待期间重连也须先核对服务端进度；未完成文件摘要失败后可从 0 重试原意图。
下载以本地临时文件实际长度续传；
完成确认单独保存并重试，已有目标文件在补发下载确认前再次核对。
页面可重试失败任务或发起取消；Qt 保存取消意图后释放连接停止当前传输，其他任务随后恢复。
服务端确认取消后才移出任务列表；确认丢失和进程重启复用原取消请求，确定失败显示“重试取消”。
取消记录与任务状态、同步事件及请求结果共同保存，初始化前取消也会阻止迟到请求恢复任务。
文件协调组件独立处理定时重试，消息发送尝试记录失败不会停止其计时器；取消请求不占用文件传输名额。
取消保留文件和已发布消息；已发布文件损坏处理及文件清理策略仍需完善。
同一上传意图由 [上传占用登记](server/quic/upload.py) 绑定到一个连接和一条文件流，流头明确续传偏移。
同设备的新连接可以接管；其他设备等待原连接释放或上传空闲超时，再核对已保存数据继续。
接管后旧流与迟到完成请求被拒绝；旧连接退出只清理自身仍持有的占用。
Qt 收到其他设备的进度时保留本机初始化重试，原文件意图和请求标识保持不变。
上传流格式与客户端需同步升级，配置及验证边界见开发指南和执行记录。

群聊使用独立成员关系，已读位置以 `conversation_members.last_read_seq` 为准；
消息未读人数由发送时的投递记录计算，通过 ReadCountUpdated 同步实际人数，
Qt 保存每条消息的最新计数，页面不按当前成员或读者位置重复扣减；旧缓存由新增纠正事件恢复。
退出成员的历史已读位置单独保留，重入恢复位置且不扩大原消息收件人范围。
撤回和阅后即焚通过同步事件传播，
...
[独立服务进程验证](tools/test_server_restart.py) 调用生产启动流程，在提交前后或文件写入边界强制终止服务，
用原数据库、证书和端口重启；Qt 自动重新登录，重试未确认操作并补齐连续同步事件。
该入口记录实际服务进程编号与退出码，检查数据库、请求身份和文件内容；机器断电及桌面页面交互需要另行验证。
