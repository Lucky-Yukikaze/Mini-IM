...
- 性能优化以已测量的瓶颈和明确收益为依据，兼顾实现与维护成本；完成收益较大的优化后收口。
  后续优先正确性、使用体验和交付；固定消息窗口及进一步容量压测等工作，待实际需求或可复现问题出现后再评估。
...
- 写入通过单一受控入口顺序执行；写队列必须承载完整业务事务，不能拆成逐条提交的 SQL。
  ...
  共享队列同时限制等待操作数与排队网络字节；拒绝入队不得执行该操作。容量耗尽后停止接收该连接的新业务，
  排空该连接已接受操作再关闭；客户端沿用已保存请求和意图恢复，内存缓冲不能算作已提交进度。
...
双向历史 loadHistoryPage 按会话内序号和消息 ID 定位，返回双端边界与剩余方向；限制页面保留数量前必须保证被移出窗口的历史可重新访问。
未读总数由 Qt 按完整本地投影和当前用户已读位置计算；页面加载历史不得增加未读数。
派生未读缓存在首次读取时从已保存数据重建，后续变化只在事务提交后发布；回滚保留原值，关闭或切换账号后重建。
...
- 共享核心或头文件变化后，先重建参与验收的全部程序；直接运行旧测试程序不能证明新实现通过。
